...
import { PanelManager, NavigationController } from "../../plugins/lib/index.ts";
...
globalThis.show_diagnostics_panel = async function (): Promise<void> {
  ...
  // Generate sample diagnostics and set them in the navigation controller
  const diagnostics = generateSampleDiagnostics();
  nav.setItems(diagnostics);

  // Open panel using PanelManager
  try {
    await panel.open({
    ...
    editor.setStatus(`Diagnostics: ${nav.count} item(s) - Press RET to jump, n/p to navigate, q to close`);
...
globalThis.diagnostics_goto = function (): void {
  ...
  if (props.length > 0) {
    ...
    // Fallback: use selected item from nav controller
...
globalThis.diagnostics_next = function (): void {
  nav.next(); // This triggers onSelectionChange -> updatePanelContent
};

// Navigation: previous diagnostic
globalThis.diagnostics_prev = function (): void {
  nav.prev(); // This triggers onSelectionChange -> updatePanelContent
};
